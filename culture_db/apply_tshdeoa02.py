"""Apply culture_db/tshdeoa02.sql using AURORA_DB_URL."""
...
def main() -> None:
    ...
    from culture_db.tshdeoa02_setup import ensure_tshdeoa02_table, sync_tshdeoa02_column_comments

    with psycopg2.connect(db_url()) as conn:
        ensure_tshdeoa02_table(conn)
        n = sync_tshdeoa02_column_comments(conn)
        conn.commit()
        with conn.cursor() as cur:
            cur.execute(
                ...
                """,
                (TSHDEOA02_SCHEMA, TSHDEOA02_TABLE),
            )
            rows = cur.fetchall()

    fq = f"{TSHDEOA02_SCHEMA}.{TSHDEOA02_TABLE}"
    print(f"OK: applied tshdeoa02.sql (table={fq}, column comments={n})")
