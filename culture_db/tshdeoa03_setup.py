...
_SQL_PATH = Path(__file__).resolve().parent / "tshdeoa03.sql"


def ensure_tshdeoa03_table(conn) -> None:
    ddl = ddl_without_grants(_SQL_PATH.read_text(encoding="utf-8"))
    with conn.cursor() as cur:
        cur.execute(ddl)


def sync_tshdeoa03_column_comments(conn) -> int:
    return sync_inst1_column_comments(conn, TSHDEOA03_TABLE, schema=TSHDEOA03_SCHEMA)
