"""Apply culture_db/tshdeoa01.sql using AURORA_DB_URL."""
from __future__ import annotations

import os
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent


def _load_dotenv() -> None:
    try:
        from dotenv import load_dotenv
    except ImportError:
        return
    for path in (ROOT / ".env.local", ROOT / ".env", Path(__file__).parent / ".env.local"):
        if path.is_file():
            load_dotenv(path, override=False)


def _read_windows_env(name: str) -> str:
    try:
        import winreg
    except ImportError:
        return ""
    for hive, subkey in (
        (winreg.HKEY_CURRENT_USER, r"Environment"),
        (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\CurrentControlSet\Control\Session Manager\Environment"),
    ):
        try:
            with winreg.OpenKey(hive, subkey) as key:
                value, _ = winreg.QueryValueEx(key, name)
                if isinstance(value, str) and value.strip():
                    return value.strip()
        except OSError:
            continue
    return ""


def db_url() -> str:
    sys.path.insert(0, str(ROOT))
    from culture_db.db_util import get_db_url
    return get_db_url()


def main() -> None:
    sys.path.insert(0, str(ROOT))
    import psycopg2

    from culture_db.table_config import TSHDEOA01_SCHEMA, TSHDEOA01_TABLE
    from culture_db.tshdeoa01_setup import ensure_tshdeoa01_table, sync_tshdeoa01_column_comments

    with psycopg2.connect(db_url()) as conn:
        ensure_tshdeoa01_table(conn)
        n = sync_tshdeoa01_column_comments(conn)
        conn.commit()
        with conn.cursor() as cur:
            cur.execute(
                """
                select column_name, data_type, character_maximum_length,
                       numeric_precision, numeric_scale, is_nullable
                from information_schema.columns
                where table_schema = %s and table_name = %s
                order by ordinal_position
                """,
                (TSHDEOA01_SCHEMA, TSHDEOA01_TABLE),
            )
            rows = cur.fetchall()

    fq = f'{TSHDEOA01_SCHEMA}.{TSHDEOA01_TABLE}'
    print(f"OK: applied tshdeoa01.sql (table={fq}, column comments={n})")
    print(f"Columns ({len(rows)}):")
    for name, dtype, char_len, num_prec, num_scale, nullable in rows:
        if char_len:
            type_label = f"{dtype}({char_len})"
        elif num_prec is not None:
            type_label = f"{dtype}({num_prec},{num_scale or 0})"
        else:
            type_label = dtype
        print(f"  - {name}: {type_label} ({nullable})")


if __name__ == "__main__":
    main()
