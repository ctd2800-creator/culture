...
_SQL_PATH = Path(__file__).resolve().parent / "tshdeoa06.sql"


def ensure_tshdeoa06_table(conn) -> None:
    ddl = ddl_without_grants(_SQL_PATH.read_text(encoding="utf-8"))
    with conn.cursor() as cur:
        cur.execute(ddl)


def sync_tshdeoa06_column_comments(conn) -> int:
    return sync_inst1_column_comments(conn, TSHDEOA06_TABLE, schema=TSHDEOA06_SCHEMA)
